...
//date of modification : 09/19/2022
...
public:
	int xres, yres;
	char keys[65536];
	// unsigned intener variable is performed a little faster
	...
	Global() {
		...
		// the mouse cursor is off(meaning not visible) initially
		mouse_cursor = 0;
...
void check_mouse(XEvent *e)
{
	...
	if (savex != e->xbutton.x || savey != e->xbutton.y) {
		//Mouse moved
		// Check mouse state 
		// if mouse cursor is on (the value is other than zero) 
		if (gl.mouse_cursor)
		    return;
		//
...
int check_keys(XEvent *e)
{
	...
	switch (key) {
		...
		// because gl.mouse_cursor has not been created
		// when Professor Griesel inserted this case below:
		case XK_m:
			...
			break;
		case XK_c:
			// Toggle credits screen on or off
			// ! is a logical operation
			// so it will flip zeroes and ones to 
			// opposite values 
			// Not zero means it is true 
			// ex) g.credits = !gl.credits;
			//
			// Exclusive ORing with 1 is 
			// 0 ^ 1 = 1
			// 1 ^ 1 = 0
			// 1 ^ 0 = 1
			gl.credits ^= 1; 
...
void render()
{
	...
	// credits page should be drawn after the game screen is drawn
	// first
	if (gl.credits) {
	    // show credits
	    show_credits();
